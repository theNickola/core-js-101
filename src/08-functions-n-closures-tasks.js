...
function getPolynom(...args) {
  const n = args.length;
  return (x) => (args[0] * (x ** (n - 1)))
    + (!args[1] ? 0 : args[1] * x ** (n - 2))
    + (!args[2] ? 0 : args[2] * x ** (n - 3));
}
...
function memoize(func) {
  // const cash = Object.create(null);
  const cash = new Map();
  return (...args) => {
    const prop = args.map((e) => e.toString()).join(',');
    if (cash.has(prop)) return cash.get(prop);
    const res = func(...args);
    cash.set(prop, res);
    return res;
  };
...
function logger(func, logFunc) {
  return (...args) => {
    const argsLog = args.map(JSON.stringify).join(',');
    logFunc(`${func.name}(${argsLog}) starts`);
    const res = func(...args);
    logFunc(`${func.name}(${argsLog}) ends`);
    return res;
  };
}
