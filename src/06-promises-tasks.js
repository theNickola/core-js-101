...
function getFastestPromise(array) {
  return Promise.race(array).then();
}
...
function chainPromises(array, action) {
  const promiseAllPoliphil = (arrayPromises) => {
    const arrayResolve = new Array(arrayPromises.length);
    return (
      new Promise((resolve, reject) => {
        for (let i = 0; i < arrayPromises.length; i += 1) {
          arrayPromises[i]
            .then((value) => { arrayResolve[i] = value; })
            .catch((ex) => reject(ex));
        }
  ...
  return new Promise(
    (resolve) => resolve(
      promiseAllPoliphil(array)
        .then((arr) => arr.reduce((acc, item) => action(acc, item)))
        .catch((ex) => ex),
    ),
  );
}
