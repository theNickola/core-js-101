...
function extractNameFromTemplate(value) {
  return value.split(',')[1].trim().slice(0, value.split(',')[1].trim().length - 1);
}
...
function removeFirstOccurrences(str, value) {
  return str.slice(0, str.indexOf(value))
...
function getRectangleString(width, height) {
  ...
  for (let i = 0; i < height; i += 1) {
    for (let j = 0; j < width; j += 1) {
      if (i === 0 && j === 0) res += '┌';
      else if (i === 0 && j === width - 1) res += '┐';
      else if (i === height - 1 && j === 0) res += '└';
      else if (i === height - 1 && j === width - 1) res += '┘';
      else if (j === 0 || j === width - 1) res += '│';
      else if (i === 0 || i === height - 1) res += '─';
      else res += ' ';
    }
    res += '\n';
  }
